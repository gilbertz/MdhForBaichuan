...
- (void)updateSizeAtIndex:(NSIndexPath *)indexPath size:(CGSize)size {
    NSMutableArray *array = [_sectionItemAttributes objectAtIndex:indexPath.section];
    UICollectionViewLayoutAttributes *attributes = [array objectAtIndex:indexPath.item];
    ...
    attributes.frame = CGRectMake(attributes.frame.origin.x, attributes.frame.origin.y,itemWidth, itemHeight);;
}
...
- (void)prepareLayout {
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        for (idx = 0; idx < itemCount; idx++) {
            ...
            NSLog(@"index item-%d row-%d section-%d", indexPath.row, indexPath.item, indexPath.section);
